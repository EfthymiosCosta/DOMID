...
assoc_detect <- function(data, marginals, target_inx, pred_inx, delta, mink_order = 1, alpha1 = 1e-3, alpha2 = 1e-1){
  ...
  if (length(vars) == 0){
    ...
    # Remove classes with 0 prob caused by factors
    ...
    for (subset in vars){
      ...
      if (all(sort(p_vals) < alpha2/(levels_num-order(p_vals)+1))){
        cat('Association found with variable', subset,'\n')
        best_subset <- subset
        aux <- TRUE
        break
      }
    ...
    if (!aux){
      ...
      # quotients of p-values & best subsets
      ...
      while (lengths >= 2){
        ...
        for (subset in sub_list){
          ...
          for (class in c(1:levels_num)){
            dt_cleaned_1 <- dt_cleaned[which(dt_cleaned[, target_inx]==class), subset]
            robpca_test <- rospca::robpca(x = dt_cleaned_1, k = ncol(dt_cleaned_1), ndir=5000)
            ...
            classes_freq <- as.vector(table(dt_cleaned[closest_pts, 1]))
            classes_freq <- classes_freq[-zero_classes]
            p_vals <- c(p_vals, stats::chisq.test(classes_freq, p = classes_prob, simulate.p.value = FALSE)$p.value)
          }
          if (all(p_vals < alpha2/(levels_num-rank(p_vals)+1))){
            max_p_vals <- c(max_p_vals, max(p_vals))
            association_subs[[length(association_subs)+1]] <- subset
          }
        }
        if (length(max_p_vals)==0){
          break
        } else {
          best_subset <- association_subs[[which.min(max_p_vals)]]
          best_subset_list[[length(best_subset_list)+1]] <- best_subset
          best_p_max <- c(best_p_max, min(max_p_vals))
          lengths <- lengths - 1
        }
      }
    }
    if ((length(best_subset) > 0 | aux)){
      if (aux){
        association_subset <- best_subset
      } else {
        association_subset <- best_subset_list[[which.min(best_p_max)]]
      }
      ...
      cat('No association detected for discrete variable', target_inx, '\n')
    }
  }
}
