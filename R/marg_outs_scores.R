...
marg_outs_scores <- function(data, disc_cols, outscorediscdf, outscorecontdf, outscorediscdfcells, rho = 0.20, epsilon = 0.02){
  ...
  if (K_max >1){
    ...
    if (K_final == 1){
      ...
      repeat{
        ...
        if (length(disc_pts_cat) < floor((rho + epsilon)*nrow(outscorediscdf))) break
      }
      ...
      repeat{
        ...
        if (K_val == 0){
          ...
          disc_pts_cat <- which(cluster_alloc!=C_0_inx)
          disc_pts_cat <- unique(disc_pts_cat)
          if (length(disc_pts_cat) > floor((rho + epsilon)*nrow(outscorediscdf))){
            disc_pts_cat <- c()
          }
          break
        }
  ...
  if (all(outscorecontdf[-disc_pts_cat, 2] < 0.65)){
    ...
    for (i in 1:length(cont_thresh_vec)){
      ...
      if (length(disc_pts_cat)+length(disc_pts_cont) <= floor((rho + epsilon)*nrow(outscorediscdf))) break
    }
