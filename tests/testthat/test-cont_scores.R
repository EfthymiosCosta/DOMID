...
testthat::test_that("Correct output dimensions.", {
  testthat::expect_equal(dim(cont_scores(data = dt,
                               cont_cols = c((disc_vars+1):(disc_vars+cont_vars)))),
               c(rows, 2))
})
testthat::test_that("Incorrect labels for continuous variables.", {
  testthat::expect_error(cont_scores(data = dt,
...
testthat::test_that("Incorrect input variable values for EIF algorithm.", {
  testthat::expect_error(cont_scores(data = dt,
                           cont_cols = c((disc_vars+1):(disc_vars+cont_vars)),
                           sample_size = 0.1),
