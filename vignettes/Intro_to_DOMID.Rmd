...
outliers_detected <- DOMID(data = dt, disc_cols = c(1:5), cont_cols = c(6:10),
                           ...
                           range_tol = 21)
all(c(unique(unlist(marginal_outliers)), joint_outliers) %in% unique(unlist(outliers_detected)))
all(unique(unlist(outliers_detected)) %in% c(unique(unlist(marginal_outliers)), joint_outliers))
```
